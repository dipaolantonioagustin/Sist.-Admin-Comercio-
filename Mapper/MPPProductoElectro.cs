using System;
using System.Collections.Generic;
using System.Data;
using System.Linq;
using System.Text;
using System.Threading.Tasks;
using Abstraccion;
using BE;
using DAL;

namespace Mapper
{
    public class MPPProductoElectro : IGestionesBase<BEProductoElectro>
    {
        Conexion conec = new Conexion();
        public bool Baja(BEProductoElectro producto)
        {
            string consulta = null;

            if (producto.codigo != 0)
            {
                consulta = $"delete from producto where producto.id_producto = {producto.codigo} ";
                return conec.Escribir(consulta);
            }
            else { return false; }
        }
        public bool Guardado(BEProductoElectro producto)
        {
            string consulta = null;
            
            if(producto.codigo != 0)
            {
                consulta = $"update producto set cantidad = {producto.cantidad}, id_proveedor = {producto.proveedor.codigo}, descripcion = '{producto.descripcion}', marca = '{producto.marca}', precio_unitario = {producto.precioUnidad}, id_medidaNombre = {producto.medicion.codigo}, medida = {producto.medida}, id_categoria = {producto.categoria.codigo} where id_producto = {producto.codigo}";
                return conec.Escribir(consulta);
            }
            else
            {
                consulta = $"insert into producto (descripcion, cantidad, marca, precio_unitario, medida, id_medidaNombre, id_proveedor, id_categoria) values ('{producto.descripcion}', {producto.cantidad}, '{producto.marca}', {producto.precioUnidad}, {producto.medida}, {producto.medicion.codigo}, {producto.proveedor.codigo}, {producto.categoria.codigo}) ";
                return conec.Escribir(consulta);
            }


            
        }

        public List<BEProductoElectro> listarTodo()
        {
            
            
            List<BEProductoElectro> listaProductos = new List<BEProductoElectro>();
            
            string consulta = "select prod.*, prov.*, med.* from producto prod join proveedor prov on prod.id_proveedor = prov.id_proveedor join medida med on prod.id_medidaNombre = med.id_medidaNombre";

            DataSet resultado = conec.Lectura(consulta);


            if (resultado.Tables[0].Rows.Count > 0) 
            {
                foreach (DataRow registro in resultado.Tables[0].Rows)
                {
                    string verificador = (registro["id_categoria"]).ToString();
                    if (verificador != "")
                    {
                        BEProductoElectro BEProductoCarga = new BEProductoElectro();

                        BEProductoCarga.codigo = Convert.ToInt32(registro["id_producto"]);
                        BEProductoCarga.descripcion = (registro["descripcion"]).ToString();
                        BEProductoCarga.marca = (registro["marca"]).ToString();
                        BEProductoCarga.precioUnidad = float.Parse(registro["precio_unitario"].ToString());
                        BEProductoCarga.medida = float.Parse(registro["medida"].ToString());
                        BEProductoCarga.cantidad = Convert.ToInt32(registro["cantidad"]);
                        

                        #region CargaProveedor
                        BEProveedor BEProveedorCarga = new BEProveedor();

                        BEProveedorCarga.codigo = Convert.ToInt32(registro["id_proveedor"]);
                        BEProveedorCarga.razonSocial = (registro["razon_social"]).ToString();
                        BEProveedorCarga.cuit = Convert.ToInt32(registro["cuit"]);

                        BEProductoCarga.proveedor = BEProveedorCarga;
                        #endregion

                        #region CargaMedicion
                        BEMedidas oBEMedidasCarga = new BEMedidas();

                        oBEMedidasCarga.codigo = Convert.ToInt32(registro["id_medidaNombre"]);
                        oBEMedidasCarga.nombre = (registro["nombre_medida"]).ToString();

                        BEProductoCarga.medicion = oBEMedidasCarga;
                        #endregion



                        #region CargaCategoria


                        //  string consulta2 = $"select categoria.nombre from categoria where categoria.id_categoria = '{(registro["id_categoria"]).ToString()}'"; // El ExecuteScalar no me toma Query que especifique campo de seleccion
                        string consulta3 = $"select *  from categoria where categoria.id_categoria = {registro["id_categoria"]}";

                        Conexion conec2 = new Conexion();

                        DataSet resultado2 = conec2.Lectura(consulta3);

                        if (resultado2.Tables[0].Rows.Count != 0)
                        {
                            foreach (DataRow registro2 in resultado2.Tables[0].Rows)
                            {
                                BECategorias oBECategoriaCarga = new BECategorias();

                                oBECategoriaCarga.nombre = registro2["nombre"].ToString();
                                oBECategoriaCarga.codigo = Convert.ToInt32(registro2["id_categoria"]);

                                BEProductoCarga.categoria = oBECategoriaCarga;

                            }       
                        }
                        #endregion 
 
                        
                        listaProductos.Add(BEProductoCarga);

                    }    
                }

            }

            return listaProductos;


        }
    }
}
